    ...
    function render(data) {
      ...
      $("agents").innerHTML = data.agents.length ? data.agents.map((agent) => `
        <button class="agent" type="button" data-agent="${esc(agent.agent_id)}" aria-pressed="${agent.agent_id === data.focused_agent_id}">
          ...
          <span class="meta" style="grid-column: 1 / -1">assistant ${esc(agent.assistant_identity_name ?? "unknown assistant")} | identity ${esc(agent.session_identity_label ?? "unknown session identity")}</span>
